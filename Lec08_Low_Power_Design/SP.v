...
//   clock gate ctl
//---------------------------------------------------------------------
reg sleep_in, sleep_gray, sleep_addsub, sleep_sma, sleep_out;

always @(*) 
begin
	if(c_state == READ || n_state == READ || c_state == STANDBY)
		sleep_in = 0;
	else
		sleep_in = 1;	
end
...
always @(*) 
begin
	if(c_state == ADDSUB || n_state == ADDSUB)
		sleep_addsub = 0;
	else
		sleep_addsub = 1;	
end
...
always @(*) 
begin
	if(c_state == RESULT || n_state == RESULT)
		sleep_out = 0;
	else
		sleep_out = 1;	
end
...
//   clock gate            
//---------------------------------------------------------------------
wire CLK_IN, CLK_GRAY, CLK_ADDSUB, CLK_SMA, CLK_OUT;
GATED_OR GATED_in(.CLOCK(clk), .SLEEP_CTRL(cg_en & sleep_in), .RST_N(rst_n), .CLOCK_GATED(CLK_IN));
GATED_OR GATED_gray(.CLOCK(clk), .SLEEP_CTRL(cg_en & sleep_gray), .RST_N(rst_n), .CLOCK_GATED(CLK_GRAY));
GATED_OR GATED_addsub(.CLOCK(clk), .SLEEP_CTRL(cg_en & sleep_addsub), .RST_N(rst_n), .CLOCK_GATED(CLK_ADDSUB));
GATED_OR GATED_sma(.CLOCK(clk), .SLEEP_CTRL(cg_en & sleep_sma), .RST_N(rst_n), .CLOCK_GATED(CLK_SMA));
...
generate
	...
	begin
		always @(posedge CLK_GRAY or negedge rst_n)
		begin
			if(!rst_n)
				step1_out[i_step1] <= 0;
...
always @(posedge CLK_ADDSUB or negedge rst_n) 
begin
	if(!rst_n)
		addsub_out <= 0;
	else
...
always @(posedge CLK_SMA or negedge rst_n)
begin
	if(!rst_n)
		sma_out <= 0;
	else
